...
public class Plan extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (requestedId == null) {
				getAllPlans(request, response, connection, JwtToken.getUserId(request));
			} else {
				getPlanById(request, response, connection, Integer.parseInt(requestedId));
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void getPlanById(HttpServletRequest request, HttpServletResponse response, PlanDBConnection connection, int id)
			throws Exception, IOException {
	...
	protected void doPut(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (connection.updatePlan(id, newName)) {
				// Write the updated plan info to the response
				getPlanById(request, response, connection, id);
				response.setStatus(200);
			} else {
